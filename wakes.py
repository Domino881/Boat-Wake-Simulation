...
def preprocess():
    ...
    for t in range(0, t_nsteps):
        ...
        # Changed the way of printing to be more pythonic, according to PEP* standard.
        print(f"\rpreprocessing: {100.0*float(t+1)/t_nsteps:.0f}%", end="")
...
for lbda in range(len(lbdas)):
    ...
    print(
        "\rlambda = %.3f  - %d/%d" % (lbdas[lbda], lbda + 1, len(lbdas)),
        end="",
        flush=True,
    )
print("")
...
ax.set(zlim=[0, 2])
ax.set_title(
    "%d lambdas in (%.2f, %.2f), " % (n_lbdas, lbdamin, lbdamax)
    + "ymax=%d, pixelsize=%.2f, t_nsteps=%d" % (ymax, pixelsize, t_nsteps)
    + ", v=%.2f" % (v)
)

# plot the surface
ax.plot_surface(X, Y, Z, cmap=plt.cm.bone, rcount=400, ccount=400, antialiased=False)
